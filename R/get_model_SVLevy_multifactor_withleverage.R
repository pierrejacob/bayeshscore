##################################################################################################
# Levy-driven Stochastic volatility model: Multi-factor with leverage
# (section 4.1 in Chopin, Jacob, Papaspiliopoulos, 2013)
##################################################################################################
#'@rdname get_model_SVLevy_multifactor_withleverage
...
#'The states are Xt = (v1t, z1t, v2t, z2t, sum_e1t, sum_e2t), where sum_e1t and sum_e2t denote the
#'sum of the error variables involved in the generation of the vt's.
#'@export
get_model_SVLevy_multifactor_withleverage <- function(timesteps,
                                                      mu0mu = 0, sigma02mu = 10,
                                                      mu0beta = 0, sigma02beta = 10,
                                                      r0xi = 1/5, r0w2 = 1/5,
                                                      r0lambda1 = 1, r0lambda2_1 = 1/2,
                                                      alpha0w = 1, beta0w = 1,
                                                      mu0rho1 = 0, sigma02rho1 = 10,
                                                      mu0rho2 = 0, sigma02rho2 = 10){
  model = list()
  ...
  model$rprior = function(Ntheta){
    ...
    rho1 = rnorm(Ntheta, mu0rho1, sqrt(sigma02rho1))
    rho2 = rnorm(Ntheta, mu0rho2, sqrt(sigma02rho2))
    return (rbind(mu, beta, xi, w2, lambda1, lambda2, w, rho1, rho2))
  }
  ...
  model$dprior = function(theta, log=TRUE){
    ...
    lrho1 = dnorm(theta[8], mu0rho1, sqrt(sigma02rho1), log=TRUE)
    lrho2 = dnorm(theta[9], mu0rho2, sqrt(sigma02rho2), log=TRUE)
    lp = lmu + lbeta + lxi + lw2 + llambda + lw + lrho1 + lrho2
  ...
  model$rinitial = function(theta,Nx){
    ...
    # lambda2 = theta[6]
    # w = theta[7]
    Xs = rinitial_SVLevy_multifactorleverage_cpp(Nx,timesteps[1],theta[3],theta[4],theta[5],theta[6],theta[7])
  ...
  model$rtransition = function(Xs,t,theta){
    ...
    # lambda2 = theta[6]
    # w = theta[7]
    new_Xs = rtransition_SVLevy_multifactorleverage_cpp(Xs, timesteps[t-1], timesteps[t], theta[3], theta[4], theta[5], theta[6], theta[7])
  ...
  model$dobs = function(Yt,Xts,t,theta,log = TRUE){
    ...
    # rho1 = theta[8]
    # rho2 = theta[9]
    return (dobs_SVLevy_multifactorleverage_cpp(Yt, Xts,
                                                theta[1], theta[2], theta[3],
                                                theta[5], theta[6], theta[7],
                                                theta[8], theta[9], log = TRUE))
  }
  ...
  model$derivativelogdobs = function(Yt,Xts,t,theta){
    ...
    # rho1 = theta[8]
    # rho2 = theta[9]
    # Note: the second derivative is the same as in the NO LEVERAGE case
    return (list(jacobian = d1logdobs_SVLevy_multifactorleverage_cpp(Yt, Xts,
                                                                     theta[1], theta[2], theta[3],
                                                                     theta[5], theta[6], theta[7],
                                                                     theta[8], theta[9]),
  ...
  model$robs = function(Xt,t,theta){
    ...
    rho2 = theta[9]
    vt = Xt[1,] + Xt[3,]
    return (rnorm(1, mu+beta*vt + rho1*Xt[5,]+rho2*Xt[6,] - xi*(w*rho1*lambda1+(1-w)*rho2*lambda2), sqrt(vt)))
  }
