...
#'@export
increase_Nx <- function(observations, t, model, thetas, xnormW, trees, algorithmic_parameters){
  ...
  for (i in 1:Ntheta){
    ...
    cpf = conditional_particle_filter(matrix(observations[,1:t],ncol = t), model, thetas[i,], algorithmic_parameters, path = current_path)
    X_new[,,i] = cpf$X
    xnormW_new[,i] = cpf$normW
