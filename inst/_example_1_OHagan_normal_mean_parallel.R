...
for (s in 1:length(sigma2prior_all)){
  ...
  for (r in 1:repl){
    results_all = rbind(results_all,data.frame(logevidence = results[[r]]$logevidence,
                                               ...
                                               sigma2prior = factor(format(sigma2prior, scientific = FALSE)),
                                               repl = r))
...
ggplot(post_all) +
  geom_density(aes(theta,weight=W,color=sigma2prior,group=interaction(sigma2prior,repl))) +
...
ggplot(results_all) +
  geom_line(aes(time, -logevidence, color = sigma2prior,group=interaction(sigma2prior,repl))) +
  scale_colour_discrete(expression(paste(" ",sigma[0]^2))) +
  ylab("- log evidence") + guides(colour = guide_legend(override.aes = list(size=2)))
#--------------------------------------------------------------------------------------------
# Check the h-score
ggplot(results_all) +
  geom_line(aes(time, hscore, color = sigma2prior,group=interaction(sigma2prior,repl))) +
  scale_colour_discrete(expression(paste(" ",sigma[0]^2))) +
  ylab("Hyvarinen score") + guides(colour = guide_legend(override.aes = list(size=2)))
#--------------------------------------------------------------------------------------------
# Check the log-evidence
ggplot(subset(results_all,time == nobservations)) +
  geom_boxplot(aes(sigma2prior,-logevidence, color = sigma2prior),outlier.shape = NA,size=1) +
  ylab("- log evidence") + xlab(expression(paste(" ",sigma[0]^2))) + theme(legend.position="none")
#--------------------------------------------------------------------------------------------
# Check the h-score
ggplot(subset(results_all,time == nobservations)) +
  geom_boxplot(aes(sigma2prior,hscore, color = sigma2prior),outlier.shape = NA,size=1) +
  scale_colour_discrete(expression(paste(" ",sigma[0]^2))) +
