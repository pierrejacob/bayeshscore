...
theta = c(0, 0, 0.5, 0.0625, 0.01) ####### FILL IN THE MISSING FIELDS !!!!!!
...
algorithmic_parameters$save_stepsize = 50
#--------------------------------------------------------------------------------------------
repl = 5 #number of replications
registerDoParallel(cores=2) #number of workers in parallel
...
ggplot(results_all,aes(time,  -logevidence/time, color = model)) +
...
# Check the h-score DDE
ggplot(results_all,aes(time, hscoreDDE, color = model)) +
  ylab("Hyvarinen score / time") +
  # guides(shape = guide_legend(override.aes = list(size=2))) +
  scale_color_manual(values = colors) +
  scale_fill_manual(values = colors) +
