#'@rdname get_model_pz5
#'@title get_model_pz5
#'@description This implements the PZ model with 5 parameters
#'@export
get_model_pz5 <- function(){
  ...
  model.pz$dprior = function(theta, log = TRUE){
    density <- dunif(theta[1],0,1, log = TRUE) + dunif(theta[2],0,1, log = TRUE) +
      dunif(theta[3],0,1, log = TRUE) + dunif(theta[4],0,1, log = TRUE) + dunif(theta[5],0,1, log = TRUE)
    if (log){
      return(density)
    } else {
      return(exp(density))
