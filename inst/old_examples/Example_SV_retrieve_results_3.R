...
setwd("C:/Users/jianfushao/Documents/Harvard/MyHarvard/_Research/Model Selection for SSM (Pierre and Jie)/Simulation/Example 6 - Stochastic Volatility (Chopin et al., 2013)/T=1000_2")
...
# ggsave("example_SV_posterior_model_1.png",plot = g1,dpi = 300,width = 10, height = 5)

g2 = do.call(grid.arrange,c(post_plot_all[[2]], ncol = 3, nrow = 3))
# ggsave("example_SV_posterior_model_2.png",plot = g2,dpi = 300,width = 10, height = 5)
...
ggplot(subset(results_all,time%in%x_resolution),aes(time, hscoreDDE, color = model)) +
  ylab("Prequential Hyv?rinen score") +
  xlab("Time") +
  xlim(0,1100) +
...
ggplot(subset(centered,time%in%x_resolution),aes(time, hscoreDDE, color = model)) +
  ylab("Prequential Hyv?rinen score (centered)") +
  # guides(shape = guide_legend(override.aes = list(size=2))) +
  xlim(0,1100) +
  xlab("Time") +
  geom_label(data = labels.df, aes(x,y,label = text,color=type), color = colors, fontface = "bold") +
